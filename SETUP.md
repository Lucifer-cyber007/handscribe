...
- **LibreOffice** (only needed for the Word to PDF tool — it shells out to
  `soffice --headless`). Install from https://www.libreoffice.org/download/
  or `winget install TheDocumentFoundation.LibreOffice` on Windows /
  `apt install libreoffice` on Debian/Ubuntu. Everything else in the app
  works fine without it; that one tool returns a 503 until it's installed.
