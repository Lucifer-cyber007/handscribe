...
def guess_bare_no_value(raw_text: str) -> str | None:
    ...
    # Last resort: OCR sometimes scrambles an entire multi-field info box
    # (Inv. No. / State / Date / Vehicle No. / Place of Supply) badly
    # enough that the value ends up many lines away from its label with
    # no reliable positional link. Given we already know a "No."-style
    # label exists somewhere, the first short standalone digit sequence
    # anywhere in the document is a reasonable guess — it's flagged
    # "Check" either way, so a wrong guess just costs a manual
    # correction rather than a silently-missing field.
    for line in lines:
        if _STANDALONE_DIGITS.match(line):
            return line
