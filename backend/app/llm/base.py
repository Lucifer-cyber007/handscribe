...
class LLMStructuringProvider(ABC):
    ...
    def translate_text(self, text: str, target_language: str) -> str:
        """Plain text translation — used as a fallback for Translate PDF when
        a document has no extractable text layer for Document Translation to
        work with (e.g. a photographed/scanned PDF), so OCR + this at least
        produces a translated result instead of a hard failure. Layout is
        not preserved here, unlike the primary Document Translation path."""
        system = (
            "You are a precise translation assistant. Translate the given "
            "text faithfully, preserving meaning and tone. Output ONLY the "
            "translated text, with nothing else before or after it — no "
            "commentary, no explanations, no markdown formatting."
        )
        prompt = (
            f"Translate the following text into {target_language} "
            f"(ISO 639-1 language code):\n\n---\n{text}\n---"
        )
        return self._call_model(system, prompt).strip()
    ...
    def _build_prompt(raw_text: str, fields: list[FieldSchemaIn]) -> str:
        ...
        for f in fields:
            ...
            if f.field_type.value == "custom_regex":
                ...
                type_desc = (
                    "date — normalize to DD/MM/YYYY (day/month/year) regardless of "
                    "how it's written in the document, e.g. '15 March 2024', "
                    "'3/15/2024', and '2024-03-15' must all be extracted as "
                    "'15/03/2024'. Use the actual date found, just reformatted."
                )
            field_lines.append(
